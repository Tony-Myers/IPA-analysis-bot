...
def ipa_analysis_pipeline(transcript, output_filename):
    ...
    if initial_notes_json:
        try:
            ...
            logger.error(f"Error parsing JSON from Stage 1: {e}")
            st.write("API Response Content:")
            st.code(initial_notes_json)
            return
    else:
    ...
    if experiential_statements_json:
        try:
            ...
            logger.error(f"Error parsing JSON from Stage 2: {e}")
            st.write("API Response Content:")
            st.code(experiential_statements_json)
            return
    else:
    ...
    if pets_json:
        try:
            ...
            logger.error(f"Error parsing JSON from Stage 3: {e}")
            st.write("API Response Content:")
            st.code(pets_json)
            return
    else:
